...
def subscriber():
	...
	channel.queue_declare(queue='jukebox')
	channel.queue_bind(exchange='microapiTree', queue='jukebox', routing_key='jukebox')
	...
	def callback(ch, method, properties, body):
		if(body != ''):	
			...
			data = {"sid":data['sid'], "microapp":"jukebox", "return":{"data":snippet}}
			data = json.dumps(data)
			connection.close()
			publisher(data)
			
	channel.basic_consume(callback, queue='jukebox', no_ack=True)
