...
def subscriber():
	...
	channel.queue_bind(exchange='microapiTree', queue='menu', routing_key='menu')
	...
	def callback(ch, method, properties, body):
		if(body != ''):	
			...
			data = {"sid":data['sid'], "microapp":"menu", "return":{"data":snippet}}
			data = json.dumps(data)
			connection.close()
			publisher(data)
			
	channel.basic_consume(callback, queue='menu', no_ack=True)
