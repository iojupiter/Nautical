...
def subscriber():
	...
	channel.queue_bind(exchange='microapiTree', queue='games', routing_key='games')
	...
	def callback(ch, method, properties, body):
		if(body != ''):	
			...
			data = {"sid":data['sid'], "microapp":"games", "return":{"data":snippet}}
			data = json.dumps(data)
			connection.close()
			publisher(data)
			
	channel.basic_consume(callback, queue='games', no_ack=True)
