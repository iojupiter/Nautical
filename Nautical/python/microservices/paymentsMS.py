...
def subscriber():
	...
	channel.queue_bind(exchange='microapiTree', queue='payments', routing_key='payments')
	...
	def callback(ch, method, properties, body):
		if(body != ''):	
			...
			data = {"sid":data['sid'], "microapp":"payments", "return":{"data":snippet}}
			data = json.dumps(data)
			connection.close()
			publisher(data)
			
	channel.basic_consume(callback, queue='payments', no_ack=True)
